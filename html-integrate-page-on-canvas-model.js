...
APELSERG.MODEL.MakeCommands = function () {
    ...
    сommands.push(new APELSERG.MODEL.Command("SHOW_COMMANDS", "Меню", baseX, baseY, lengthX, lengthY, color));
    ...
    сommands.push(new APELSERG.MODEL.Command("TEXT_SPEED_HTML", "Показать/скрыть HTML ввод", baseX, baseY, lengthX, lengthY, color));

    baseY += lengthY + spaceY;
    сommands.push(new APELSERG.MODEL.Command("TEXT_SPEED_HYBRID", "Показать/скрыть гибридный ввод", baseX, baseY, lengthX, lengthY, color));
...
APELSERG.MODEL.MakeContent = function () {
    ...
    var color = "white";
    var pointX = 0;
    var pointY = APELSERG.CONFIG.PROC.CanvaID.height;
    var addY = 30;
    var Cnt = 0;
...
APELSERG.MODEL.CommandHandle = function (command) {
    ...
    if(APELSERG.MODEL.CheckClickFrame(command) && !APELSERG.CONFIG.PROC.Pause) {
        ...
        if (command.Code == "SHOW_COMMANDS") {
            ...
            //APELSERG.CONFIG.PROC.ShowCommands ? command.Name = 'Скрыть' : command.Name = 'Меню';
        }
    }
    ...
    if (APELSERG.MODEL.CheckClickFrame(command) && APELSERG.CONFIG.PROC.ShowCommands && !APELSERG.CONFIG.PROC.Pause) {
        ...
        if (command.Code == "STOP") {
            ...
            //-- показать красный бордюр перед остановкой
            //--
        ...
        if (command.Code == "TEXT_SPEED_HYBRID") {
            
            command.SelectCnt = APELSERG.CONFIG.SET.CntSelect; //-- показать красный бордюр 

            APELSERG.MAIN.ShowSettingsHybrid();
        }
        if (command.Code == "TEXT_SPEED_HTML") {

            command.SelectCnt = APELSERG.CONFIG.SET.CntSelect; //-- показать красный бордюр 

            APELSERG.MAIN.ShowSettingsHtml();
        }
