...
APELSERG.MAIN.OnLoad = function () {
    ...
    APELSERG.CONFIG.PROC.Img.onload = function () {
        ...
        //if (APELSERG.CONFIG.PROC.ShowCommands) APELSERG.MODEL.DATA.Commands[0].Name = "Скрыть";
        //else APELSERG.MODEL.DATA.Commands[0].Name = "Меню";
        ...
        APELSERG.CONFIG.PROC.CanvaID.addEventListener('click', function (event) {
            ...
            //-- тоже пропускает бордер
            //--
            //var rect = APELSERG.CONFIG.PROC.CanvaID.getBoundingClientRect();
            //APELSERG.CONFIG.PROC.MouseClickX = event.clientX - rect.left;
            //APELSERG.CONFIG.PROC.MouseClickY = event.clientY - rect.top;
        });
...
APELSERG.MAIN.CanvasSize = function () {
    ...
    var hybridDiv = document.getElementById('APELSERG_DivUiHybrid');
...
//===
APELSERG.MAIN.ShowSettingsHtml = function () {

    var htmlDiv = document.getElementById('APELSERG_DivUiHtml');

    if (htmlDiv.innerHTML === undefined || htmlDiv.innerHTML == "") {

        htmlDiv.innerHTML = "Тема" +
            "<select id='APELSERG_UserComboBoxHtml'>" +
            ...
            "</select>" +
            "<input type='button' value='Применить' onclick='APELSERG.MAIN.ApplySettingsHtml();' />";

        document.getElementById('APELSERG_UserComboBoxHtml').value = APELSERG.CONFIG.SET.ThemeSelector;
    }
    else {
        htmlDiv.innerHTML = "";
    }
...
//===
APELSERG.MAIN.ApplySettingsHtml = function () {

    APELSERG.CONFIG.SET.ThemeSelector = parseInt(document.getElementById('APELSERG_UserComboBoxHtml').value);
    ...
    document.getElementById('APELSERG_DivUiHtml').innerHTML = "";
    APELSERG.MAIN.OnLoad();
}

//===
// Показать настройки через гибрид
//===
APELSERG.MAIN.ShowSettingsHybrid = function () {
    ...
    if (hybridDiv.style.zIndex != "1") {
        ...
        hybridDiv.innerHTML = "Текст" +
            "<select id='APELSERG_UserComboBoxHybrid'>" +
                "<option value='1'>Медленно</option>" +
                "<option value='3'>Нормально</option>" +
                "<option value='5'>Быстро</option>" +
                "<option value='7'>Очень быстро</option>" +
            "</select>" +
            "<input type='button' value='Применить' onclick='APELSERG.MAIN.ApplySettingsHybrid();' />";

        hybridDiv.style.zIndex = "1"
        document.getElementById('APELSERG_UserComboBoxHybrid').value = APELSERG.CONFIG.SET.ContentMove;
    }
    else {
        hybridDiv.innerHTML = "";
        hybridDiv.style.zIndex = "-1"
...
// Применить изменение контента
//===
APELSERG.MAIN.ApplySettingsHybrid = function () {

    APELSERG.CONFIG.SET.ContentMove = parseInt(document.getElementById('APELSERG_UserComboBoxHybrid').value);
...
APELSERG.MAIN.Start = function () {
    ...
    if (APELSERG.CONFIG.PROC.Stop) {
        ...
        APELSERG.MODEL.DATA.Commands[1].SelectCnt = APELSERG.CONFIG.SET.CntSelect;
        APELSERG.CONFIG.PROC.CntStop = APELSERG.CONFIG.SET.CntSelect;
    }
}
